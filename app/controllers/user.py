...
from app.utils.common import (
    id_generator, create_access_token, verify_password,
...
class UserController:
    ...
    def token_required(self, token: str, user_id: int):
        return self.session.query(TokenTable).filter_by(
                    user_id=user_id,
                    access_toke=token,
                    status=True
        ).first()
    ...
    def logout(self, token: str, user_id: int):
        ...
        if info:
            existing_token = self.session.query(TokenTable).where(TokenTable.user_id.in_(info)).delete()
            self.session.commit()

        existing_token = self.session.query(TokenTable).filter(TokenTable.user_id == user_id, TokenTable.access_toke==token).first()
        if existing_token:
